...
class Manager():
    ...
    def save(self, obj):
        ...
        # set number of "%s ," equal to the number of the object's params:
        values_qty = "%s, " * len(params)
        values_qty = values_qty.strip()[:-1:]  # rm ending space -> ending ","
        if len(params) == 1:
    ...
    def save_all(self, objects_list):
        ...
        for obj in objects_list:
            ...
            obj_insertion = f"""INSERT INTO {obj_table} ({obj.object_attributes_to_str()})
                                VALUES ({obj.object_values_to_str()})"""
            ...
            for attribute in obj.__dict__.keys():
                if type(obj.__dict__[attribute]) is list:
                    for obj in obj.__dict__[attribute]:
                        ...
                        obj_2nd_insertion = f"""INSERT INTO {obj_2nd_table} ({obj.object_attributes_to_str()})
                                                VALUES ({obj.object_values_to_str()})
                        ...
                        m_to_m_insertion = f"""INSERT INTO {m_to_m_table} ({obj_table}_id, {obj_2nd_table}_id)
                                               VALUES (@obj_id, @obj_2nd_id)"""
    ...
    def all(self):
        ...
        query = f"""SELECT {parent_class_cols[:-1]} """ \
                f"""FROM {parent_class_table}"""
        self.db.cursor.execute(query)
        parent_class_rows = self.db.cursor.fetchall()  # [(14, 'Yaourts'),...]
    ...
    def filter(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if '__' in key and '__lt' not in key:
                second_table = key[:key.index('_')]  # set the snd_table name
                condition_key = key.replace('__', '.')
                # ex: 'category__name' -> 'category.name'
                filters += f" {condition_key} = '{value}' AND "
            # check if a condition 'less_than' exists
            elif '__lt' in key:
                condition_key = key.replace('__lt', '')
                # ex: 'nutriscore__lt' -> 'nutriscore'
                filters += f" {condition_key} < '{value}' AND "
            else:
                filters += f" {key} = '{value}' AND "
        filters = filters[:-5]  # delete the last "AND"

        # set up the first part of the query
        # depending on whether there is a second table or not
        if second_table != "":  # if there is a second table
            ...
            select = f"""SELECT {main_table_elements}{second_table_elements[:-1]}
                         ...
                         ON {second_table}.id = {main_table}_{second_table}.{second_table}_id"""
        ...
        # add 1st and snd part of the query in order to set up the final query
